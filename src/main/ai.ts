...
- **TUI 自动化**：遇到交互式安装器、菜单、编辑器、REPL、全屏 TUI 或需要键盘选择的软件时，不要用 \`execute_command\` 等待它结束。应打开/复用终端后按 \`observe_terminal -> send_terminal_keys -> wait_terminal_text/observe_terminal\` 循环自动完成操作，并在关键选择前后观察屏幕。
