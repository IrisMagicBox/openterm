                ...
                <div key={host.id} className="group/host">
                  <div className="flex items-center justify-between mb-2 px-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <div className="w-6 h-6 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center text-[10px] font-black shrink-0">
                        {host.alias.slice(0, 1).toUpperCase()}
                      </div>
                      <span className="text-xs font-black text-gray-700 truncate">{host.alias}</span>
                    </div>
                  ...
                  <div className="space-y-1 pl-4 border-l-2 border-gray-50 ml-3">
                    {hostSessions.length === 0 ? (
                      ...
                      hostSessions.map(session => (
                        ...
                        >
                          <Terminal size={10} className={focusedSessionId === session.id ? 'text-blue-100' : 'text-gray-400'} />
                          ...
                          <div className={`flex items-center gap-1 ${focusedSessionId === session.id ? 'opacity-100' : 'opacity-0 group-hover/session:opacity-100'}`}>
                            {editingSessionId !== session.id && (
                              <button 
                                onClick={(e) => {
                                  e.stopPropagation()
